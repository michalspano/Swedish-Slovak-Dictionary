...
class Lesson4EndViewController: UIViewController {
    ...
    @IBOutlet weak var gradientView: GradientViewLesson4!
    
    var lessonID = Int()
    var lessonIdentifier3 = String()
    ...
    func lessonAssets() {
        ...
        label2.text = "Lesson \(lessonID) was completed."
        quizButton.setTitle("Lesson \(lessonID) Quiz", for: .normal)
    }
    func loadGifView() {
    ...
    func initialDisplacement(){
        ...
        quizButton.transform = CGAffineTransform(translationX: 0, y: -100)
    ...
    @IBAction func didTapQuizButton(_ sender: Any) {
        performSegue(withIdentifier: "QuizLesson\(lessonID)", sender: (Any).self)
    }
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if (segue.identifier == "QuizLesson4") {
            let lesson4Quiz = segue.destination as! Lesson2Quiz
            lesson4Quiz.lessonIdentifier = "Lesson4"
        }
        else if (segue.identifier == "QuizLesson5") {
            let lesson5Quiz = segue.destination as! Lesson1Quiz
            lesson5Quiz.lessonIdentifier = "Lesson5"
        }
    }
}
